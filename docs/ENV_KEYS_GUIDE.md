...
## 17. `SMTP_USER` (Optional)

**What:** Your Gmail address used to send password reset emails.
...
- Uses Gmail SMTP (`smtp.gmail.com:587`) via nodemailer

**How to get it:**
1. Enable **2-Step Verification** on your Google account
2. Go to Google Account → Security → **App passwords**
3. Generate a new App Password for "Mail"
4. Use your Gmail address as `SMTP_USER` and the 16-character app password as `SMTP_PASS`

**Required only if** you want actual password reset emails to be sent to users.

---

## 18. `SMTP_PASS` (Optional)

**What:** Gmail App Password (16 characters) for SMTP authentication.

**How it works:**
- This is **not your real Gmail password** — it's a separate app-specific password generated in Google Account settings
- Format: `xxxx xxxx xxxx xxxx` (spaces are fine, Gmail ignores them)

**Required only if** `SMTP_USER` is set.

---

## 19. `EMAIL_FROM`

**What:** The sender name and address shown in password reset emails.

**Default:** falls back to `SMTP_USER` if not set

**How it works:**
- Can be a plain email (`you@gmail.com`) or include a display name (`WhatsApp Bot <you@gmail.com>`)
- Example: `WhatsApp Bot <chamaradilshan0011@gmail.com>`
...
| `SMTP_USER` | API → Gmail SMTP | Password reset email delivery (optional) |
| `SMTP_PASS` | API → Gmail SMTP | Gmail App Password for authentication (optional) |
| `EMAIL_FROM` | API → Gmail SMTP | Sender name/address for reset emails (optional) |
